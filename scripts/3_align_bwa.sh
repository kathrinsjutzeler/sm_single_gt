...
for sample in $(cat 
/master/kbailey/sm_single_gt/data/seq_data/test_list.txt);
  do
  SAM=$(echo ${sample} | cut -f1 -d"_")

  CMD="bwa mem -M $REF_DIR/GCA_000237925.5_SM_V9_genomic.fna.gz 
$RESULTS_DIR/trimmed_fastq/${sample}_R1_001_val_1.fq.gz $RESULTS_DIR/trimmed_fastq/${sample}_R2_001_val_1.fq.gz 
| samtools sort -o \"$RESULTS_DIR/aligned/${sample}.bam\"
